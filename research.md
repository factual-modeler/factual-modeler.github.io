...
**执行层**：在真实执行前，智能体在想象空间中展开多条候选动作序列，评估预测结果（包括奖励和安全信号），选择最优且风险最低的方案下发给机器人。

这种将"感知理解"（VLA）与"规划决策"（世界模型 + 因果推理）分离的方法，显著提升了任务成功率、样本效率，并大幅降低了不安全行为。实验表明，我们的方案超越纯端到端 VLA。
